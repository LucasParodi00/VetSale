...
//Se modifico para poder calcular automáticamente el precioVenta y precioSuelto.
const setProductos = async (req, res) => {
    try {
        const { body, file } = req;
        const { codEdades, codMascotas } = body;

        // Calcular el precioVenta y el precioSuelto
        body.precioVenta = (parseFloat(body.precioCompra) * 1.3).toFixed(2);
        body.precioSuelto = (parseFloat(body.precioCompra) / 10 * 1.1).toFixed(2);
